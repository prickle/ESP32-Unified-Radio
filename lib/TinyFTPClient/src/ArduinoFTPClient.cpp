...
bool FTPBasicAPI::restart(uint32_t pos) {
    char buffer[80];
    FTPLogger::writeLog( LOG_DEBUG, "FTPBasicAPI","restart");      
    sprintf(buffer, "%d", pos);
    return cmd("REST", buffer, "350");
}

Client *FTPBasicAPI::read(const char* file_name ) {
...
Client *FTPBasicAPI::write(const char* file_name, FileMode mode) {
...
Client* FTPBasicAPI::ls(const char* file_name){
    FTPLogger::writeLog( LOG_DEBUG, "FTPBasicAPI", "ls");      
    abort();
    const char* ok[] = {"125", "150", nullptr};
    if (!cmd("NLST",file_name, ok)) return nullptr;
    current_operation = LS_OP;
    return data_ptr;
}

Client* FTPBasicAPI::dir(const char* file_name){
    FTPLogger::writeLog( LOG_DEBUG, "FTPBasicAPI", "ls");      
    abort();
    const char* ok[] = {"125", "150", nullptr};
    if (!cmd("LIST",file_name, ok)) return nullptr;
    current_operation = LS_OP;
    return data_ptr;
}
...
bool FTPBasicAPI::connect(IPAddress adr, int port, Client *client_ptr, bool doCheckResult  ){
    ...
    if (!ok){
        FTPLogger::writeLog( LOG_ERROR, "connect", buffer);      
    }
    return ok;
}
...
bool FTPBasicAPI::checkResult(const char* expected[],const char* command, bool wait_for_data) {
    ...
    if (wait_for_data || stream_ptr->available()) {
        ...
        while(stream_ptr->available()) stream_ptr->read();
    ...
    // log error
    if (!ok){
        FTPLogger::writeLog( LOG_ERROR, command, result_reply);
    }
    return ok;    
}
