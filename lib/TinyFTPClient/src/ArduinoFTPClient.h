...
public:
  ...
  virtual bool abort();
  virtual bool restart(uint32_t pos);
  virtual Client *read(const char *file_name);
  virtual Client *write(const char *file_name, FileMode mode);
  virtual Client *ls(const char *file_name);
  virtual Client *dir(const char *file_name);  
