...
extern "C" void dma_end_callback(spi_transaction_t *);
/***************************************************************************************
...
void     (*dmaCallbackFunction)(void) = nullptr;

void TFT_eSPI::setCallbackDMA(void(*fn)(void)) {
  dmaCallbackFunction = fn;
}

void IRAM_ATTR dma_end_callback(spi_transaction_t *spi_tx)
{
  if (dmaCallbackFunction) (*dmaCallbackFunction)();
}
