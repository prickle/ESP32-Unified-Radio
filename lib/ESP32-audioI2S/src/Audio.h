...
extern __attribute__((weak)) void audio_set_sample_rate(uint32_t rate);
...
private:
    ...
    char            chbuf[4096 + 128];               // must be greater than m_lastHost #254
    char            m_lastHost[4096];                // Store the last URL to a webstream
    ...
    bool            m_metadata = true;              // Ask for ICY Metadata - Nick Metcalfe 12/2024
