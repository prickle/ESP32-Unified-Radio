...
//Redirects come here and reuse the metadata flag - Nick Metcalfe 12/2024
bool Audio::connecttohost(const char* host) {
    return connecttohost(host, "", "", m_metadata);
}
...
bool Audio::connecttohost(const char* host, const char* user, const char* pwd, bool metadata) {
    ...
    if(pos_slash > 1) {
        ...
        if (m_f_psram) hostwoext = (char*)ps_malloc(hostwoextLen + 1);        //Nick Metcalfe 2/24 - save memory
        else hostwoext = (char*)malloc(hostwoextLen + 1);
        memcpy(hostwoext, h_host, hostwoextLen);
        hostwoext[hostwoextLen] = '\0';
        uint16_t extLen =  urlencode_expected_len(h_host + pos_slash);
        if (m_f_psram) extension = (char *)ps_malloc(extLen);        //Nick Metcalfe 2/24 - save memory
        else extension = (char *)malloc(extLen);
    ...
    if (!metadata && audio_info) audio_info("Spoofing Mozilla/5.0");

    char* resp; //[strlen(h_host) + strlen(authorization) + 500];   //Nick Metcalfe 2/24 - save memory - this got big
    if (m_f_psram) resp = (char*)ps_malloc(strlen(h_host) + strlen(authorization) + 500);
    else resp = (char*)malloc(strlen(h_host) + strlen(authorization) + 500);
    resp[0] = '\0';
...
//    strcat(resp, "Transfer-Encoding: \r\n");  // otherwise the server assumes gzip compression
    //Nick Metcalfe 2/2024 fix for "open.live.bbc.co.uk" returning 403 forbidden
      ...
      strcat(resp, "Accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,*/*;q=0.8\r\n");
      strcat(resp, "Accept-Language: en-US,en;q=0.5\r\n");
      strcat(resp, "Accept-Encoding: gzip, deflate\r\n");
      strcat(resp, "Upgrade-Insecure-Requests: 1\r\n");
//    strcat(resp, "Transfer-Encoding: \r\n");  // otherwise the server assumes gzip compression
...
bool Audio::connecttoFS(fs::FS &fs, const char* path, uint32_t resumeFilePos) {
    ...
    if(!audiofile) {
        ...
        char *afn =strdup(audioName); // store temporary the name
        AUDIO_INFO(sprintf(chbuf, "Invalid file \"%s\"", afn);)
    ...
    if(endsWith(afn, ".m4a") || endsWith(afn, ".mp4")){      // M4A section, iTunes
        free(afn);
        m_codec = CODEC_M4A;
...
int Audio::read_FLAC_Header(uint8_t *data, size_t len) {
    ...
    if(m_controlCounter == FLAC_MAGIC) { /* check MAGIC STRING */
        ...
        if(specialIndexOf(data, "fLaC", 10) != 0) {
            if(specialIndexOf(data, "ID3", 4) == 0) { // ID3 tag!
              AUDIO_INFO(sprintf(chbuf, "Skipping ID3 tag!");)
              offset = 10 + bigEndian(data + 6, 4, 7);
              if (*(data + 5) & 0x10) offset += 10;
            }
            if(specialIndexOf(data + offset, "fLaC", 10) != 0) {
              log_e("Magic String 'fLaC' not found in header, found '%s'", data);
              //stopSong();
              return -1;
            }
            offset += 4;
...
/*
    ...
    if(m_controlCounter == M4A_FTYP) { /* check_m4a_file */
        ...
        int mp4  = specialIndexOf(data, "mp4", 20);

        if((m4a !=8) && (isom != 8) && (mp4 != 8)){
            log_e("subtype 'MA4 ' or 'isom' or 'mp4' expected, but found '%s '", (data + 8));
            stopSong();
            return -1;
        }
        ...
        if(m_controlCounter != 100){
            ...
            if(m_codec == CODEC_FLAC){
                ...
                if(res >= 0) bytesDecoded = res;
                    ...
                    //stopSong();
                    log_e("Invalid audio header, skipped");
                    m_controlCounter = 100;
                }
            }
            if(!isRunning()){
...
*/
    ...
    if(InBuff.bufferFilled() == 0 && f_stream && !f_webFileDataComplete){  //(< maxFrameSize) Nick Metcalfe 5/24 - some starved streams play ok, look for actual dropouts
    ...
    bool b = (m_audioDataSize  > 0) && (m_audioDataSize <= m_audioDataCount + maxFrameSize);
    if(!a && !b) return; // 2. guard   fill < frame && last frame(s)

    size_t data2decode = InBuff.bufferFilled();

    if(data2decode < maxFrameSize){
        if(m_audioDataSize - m_audioDataCount < maxFrameSize){
            data2decode = m_audioDataSize - m_audioDataCount;
        }
        else return;
    }
    ...
    if(m_f_webfile){
        ...
        if(bytesDecoded > 0) m_audioDataCount += bytesDecoded;
        ...
        if(m_audioDataSize == m_audioDataCount &&  m_controlCounter == 100) f_webFileAudioComplete = true;
    }
    else { // not a webfile
    ...
    if(bytesDecoded < 0) {  // no syncword found or decode error, try next chunk
        ...
        if(m_f_webfile) m_audioDataCount += next;
        return;
    }
    else {
        if(bytesDecoded > 0) {InBuff.bytesWasRead(bytesDecoded); return;}
...
uint32_t Audio::getFileSize() {
    if(m_f_webfile) return m_audioDataSize; //Nick Metcalfe 2/2024 for podcast progressbar
    if(!audiofile) return 0;
    return audiofile.size();
}
//---------------------------------------------------------------------------------------------------------------------
uint32_t Audio::getFilePos() {
    if(m_f_webfile) return m_audioDataCount;   //Nick Metcalfe 2/2024 for podcast progressbar
    if(!audiofile) return 0;
...
bool Audio::audioFileSeek(const float speed) {
    ...
    if(audio_set_sample_rate) audio_set_sample_rate(srate);
    return true;
}
//---------------------------------------------------------------------------------------------------------------------
bool Audio::setSampleRate(uint32_t sampRate) {
    ...
    if(audio_set_sample_rate) audio_set_sample_rate(m_sampleRate);
